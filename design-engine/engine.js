...
"""`;
  ...
  const VIDEO_TYPES = {
    routine:   { label:'🪥 Silent Routine + VO (render-safe default)', style:'lofi_native', postCaptions:true, narrative:'a SILENT DAILY-ROUTINE ad built as TWO kinds of shot that ALTERNATE and are NEVER combined (this separation is what makes it render cleanly — it removes every motion the AI hallucinates):\n  • PERSON shots: ONE person silently does ONE concrete everyday grooming task — actively drying their face with a towel, splashing water on their face, brushing their hair, buttoning a shirt, applying light moisturiser to their cheeks — natural, unhurried, NOT talking and NEVER looking at camera. In a PERSON shot the LACALUT product is NOT in frame at all (no counter pack, nothing) — so there is never a hand near it to grab.\n  • PRODUCT shots: the LACALUT pack sits alone on the counter / sink edge as a calm still-life hero, with NO person in frame (or only a soft out-of-focus torso far in the background, hands nowhere near it). The camera is essentially LOCKED OFF — a tripod-still hold or the gentlest slow drift — so the pack never moves, slides or morphs.\n  STRUCTURE: MOST scenes are PERSON shots — keep the ad alive, the person is ALWAYS actively doing something (never idle) — and the product gets exactly ONE dedicated PRODUCT hero shot as the FINAL scene, carrying the spoken CTA (for a longer ad you may add ONE more product shot in the middle, but never make the whole ad static product holds). You do NOT need the product in every scene — the VOICEOVER keeps it present; showing it once, cleanly, at the end is enough. The product is NEVER touched, picked up, opened, squeezed or dispensed, and no toothpaste, paste or droplet EVER appears. The VOICEOVER carries 100% of the message. Absolutely no demo, mirror or VFX. Each scene = ONE simple concrete action (person) or ONE still locked-off product hold (product) — never a vague "looking pensive" / "playing with her face" / "standing there" beat; the stiller and simpler the shot, the cleaner it renders.',
  ...
  function lintStoryboard(sb, styleId, typeId, sku){
    ...
    (sb.scenes||[]).forEach((sc,i)=>{
      ...
      // PERSON/PRODUCT SEPARATION — active hands + the pack in ONE scene is what makes the video model
      // grab the pack (the 27/09 failure). Block it before spend: pack and an active hand never co-occur.
      const scVM=String(sc.visual||'')+' '+String(sc.motion||'');
      const hasProd=/\b(pack|tube|box|bottle|product|lacalut)\b/i.test(scVM);
      const activeHands=/\b(dry\w*|dab\w*|towel|wash\w*|splash\w*|brush\w*|shav\w*|appl(?:y|ies|ying)|serum|moisturi[sz]\w*|rins\w*|button\w*|comb\w*|wipe\w*|reach\w*|grab\w*|pick\w*|hold\w*|lift\w*|squeez\w*|press\w*)\b/i.test(scVM);
      if(hasProd && activeHands) out.push('Scene '+(i+1)+': the pack AND an active-hand action share ONE scene — this is exactly what makes the video model grab the pack. SEPARATE them: either drop the pack (person-only action) OR drop the hand action (product-only still-life). A hand and the pack must never be in the same scene.');
      // IDLE person scene — no concrete action reads as a boring, broken clip (Quan's 27/09 note).
      ...
      if(hasPerson && !activeHands && !hasProd) out.push('Scene '+(i+1)+': the person has no concrete action (idle / looking / smiling / standing only) — every person scene must show ONE specific everyday task actively happening (drying face, splashing water, brushing hair, applying moisturiser). A person doing nothing renders as a boring, broken clip.');
    });
...
- PERSON/PRODUCT SEPARATION (hard rule — the #1 reason renders fail): a single scene shows EITHER the person doing their action OR the product, NEVER both together when the person's hands are active. If a scene shows the person washing / drying / brushing / applying anything, the LACALUT pack is NOT in that frame at all — because the moment a hand and the pack share a frame the video model bridges them into an unwanted grab (the exact failure to avoid). The pack lives in its OWN dedicated PRODUCT shots with no person (or only a soft, distant, out-of-focus background torso, hands nowhere near it). Write each scene as clearly one kind or the other in its "visual".
- STILL PRODUCT SHOTS (hard rule): in a PRODUCT shot the pack sits on the counter as a real photographed still-life and the camera is LOCKED OFF (tripod-still or the gentlest slow drift) — the pack NEVER slides, repositions, floats, or moves from its spot; motion is only in the light or a whisper of camera drift, never in the pack.
- CONCRETE ACTION (hard rule): every PERSON scene\'s "visual" and "motion" must describe ONE specific, continuous, physical everyday action the person is actively DOING (drying face with a towel, splashing water, brushing hair, buttoning a shirt, smoothing moisturiser onto a cheek) — NEVER a passive "standing", "looking pensive", "gazing", "playing with her face" or "holding an expression" beat with no real action. A person doing nothing is boring and reads as a broken clip.
...
Return ONLY this one scene as JSON, exactly this schema: {"visual":"...","motion":"...","vo":"...","text":"..."}`;
  ...
  function buildScenePrompt(opts){
    ...
    if(personlessScene && hasPack){
      ...
      p += `PRODUCT STAYS PUT (hard rule): the pack rests on its surface and does NOT move, slide or reposition during the clip; no hand enters to touch, lift or grab it — hands and the pack never make contact. `;
    }
  ...
  async function renderScene(opts){
    ...
    try{
      ...
      if(sceneWantsPack && packRefs.length && scenePersonless){
        // PRODUCT-ONLY still-life — generate NATIVELY with the packshot as reference so the pack is a
        // real photographed object baked into the scene's own light and surface (never a pasted cut-out).
        onProg('🧴 composing product still-life…');
        const spHero = sp + `PRODUCT STILL-LIFE (hard rule for THIS frame): the LACALUT pack rests on the counter as a REAL photographed object that is genuinely PART of this photo — sitting in the scene's own light, casting a soft natural contact shadow onto the surface, with the same focus, grain and colour grade as everything around it (as believably real as the plant or the basin beside it). It must NEVER look pasted, cut-out, flat, floating or 2D. LOCKED-OFF: the pack sits perfectly still in its spot. Reproduce the reference packshot EXACTLY — real German label, WHITE cap, true tall-slim proportions.`;
        try{
