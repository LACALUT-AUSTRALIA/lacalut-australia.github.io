...
"""`;
  ...
  const SB_LINT_RULES = [
    ...
    { re:/\b(snap zoom|zoom(s|ing)?|push(es|ing)?)\b[^.]{0,40}\b(label|pack text|wording|logo)\b/i, f:['visual','motion'], msg:'camera zooming onto the label — mid-motion text zooms warp the lettering; keep camera moves on the pack slow and gentle, the still guarantees label fidelity' },
    ...
    { re:/\b(aura|energy|halo|glow(s|ing)?|shimmer\w*)\b[^.]{0,45}\b(pack|tube|box|product|paste)\b|\b(pack|tube|box|product)\b[^.]{0,45}\b(aura|energy|halo)\b|\benvelop\w+/i, f:['visual','motion'], msg:'energy/aura/glow enveloping the pack — the banned halo effect; crystals may move NEAR the pack but never wrap or envelop it' },
    { re:/\b(pack|tube|box|product)\b[^.]{0,50}\b(glides?|gliding|slides?|sliding|floats?|floating|drifts?|transitions?|moves? (in|into|across)|bursts? (in|into)|pops? (in|into)|flies|flying)\b/i, f:['visual','motion'], msg:'the pack moving itself (glide/slide/float/fly-in) — the pack is ALWAYS static and already in place; the camera or environment moves, never the pack' },
    ...
    { re:/\b(water|liquid|stream|droplets?|drips?|pours?|pouring)\b[^.]{0,55}\b(crystals?|shards?|minerals?)\b|\b(crystals?|shards?)\b[^.]{0,40}\b(water|stream|drips?)\b/i, f:['visual','motion'], msg:'water interacting with crystals/minerals — the rejected rock-water fault; water exists ONLY as tap-into-sink or a drinking glass' },
    ...
    { re:/\b(gums(?! care)|gum ?line)\b/i, f:['visual','motion'], msg:'gums visually depicted — gums are NEVER shown on screen (uncanny); they live in the voiceover and captions only' },
    ...
    { re:/\b(crystals?|shards?)\b[^.]{0,25}\b(paste|ribbon|toothpaste)\b|\b(paste|toothpaste)\b[^.]{0,35}\b(crystals?|shards?|integrated|embedded|studded)\b/i, f:['visual','motion'], msg:'crystals inside/on the paste — paste is always pure, smooth, glossy white; formula elements live NEAR the product, never in the paste' },
    ...
    { re:/\b(zoom\w*|push\w*|close-?up|CU|ECU|macro)\b[^.]{0,50}\b(plant|pot|towel|mug|cup|glass|tumbler|window|prop|shelf)\b/i, f:['visual','motion'], msg:'camera feature on a PROP — every beat must serve the product story; cut the prop beat and give the time to the problem or the pack' },
    ...
    { re:/\b(?<!feels )(?<!feel )firm\w*[^.]{0,15}?gums?\b/i, f:['vo','text'], msg:'bare firming-result phrase ("firm gums") — feel-language only: "gums that FEEL firm"' },
...
- AI-WEAKNESS AVOIDANCE (hard rules — these shots ALWAYS render badly): NEVER write mirror-reflection shots (reflections mangle the pack and the person); NEVER extreme face close-ups filling the frame (uncanny valley) — nothing tighter than a chest-up mid-shot on a person, no mouth-only or teeth-macro framing, NEVER a tongue; keep hands minimal and simple — at most ONE simple hand action per scene (pick up, put down, hold, turn a tap), never twirling/fiddling/tossing objects, never fine finger actions and never touches the mouth, jaw, chin, cheek or any part of the face; NEVER overlay visual effects (gels, mists, glows, particles) on a person's face or skin — effects live in the environment, away from people; the product is lit naturally, never wrapped, enveloped or surrounded by a glow/halo/energy effect — crystals and streams move NEAR the pack, never around or onto it.
...
TYPE CHECKLIST (structural musts): Problem-Agitate-Solve = ~60% of runtime on the problem, product appears only after; Demo = the product physically IN ACTION in macro; Old-vs-New = BOTH sides actually shown in contrast; Transformation = an explicit BEFORE beat and AFTER beat (no before/after = fail, restructure); Social-Proof Mashup = rapid micro-beats each landing ONE captioned benefit; Brand Story = no people on camera, heritage narrative. ALSO: the character wears THE SAME outfit in every scene AND on both sides of any split/contrast (an outfit change between sides is a fail); no scene packs more than 3 distinct shots into its 8 seconds; paste never dispenses itself; the WORLD must be physically concrete, and for contrast formats it must describe BOTH settings in detail.
