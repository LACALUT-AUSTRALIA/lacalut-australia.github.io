...
"""`;
  ...
  const VIDEO_TYPES = {
    routine:   { label:'🪥 Silent Routine + VO (render-safe default)', style:'lofi_native', postCaptions:true, narrative:'a SILENT DAILY-ROUTINE ad (the render-safe format that plays to the AI model\'s strengths and avoids everything it fails at): the LACALUT product simply RESTS on the counter / sink edge the entire time and is NEVER touched, picked up, opened, squeezed or dispensed; ONE person silently does ONE calm ordinary everyday grooming task — brushing their teeth, washing or splashing their face, shaving, drying their face with a towel, applying skincare or light makeup — natural and unhurried, NOT talking and NEVER looking at the camera (like the aesthetic "silent morning-routine" reels). The VOICEOVER carries 100% of the message over the top — the visuals are just a real, calm human moment. Absolutely NO product handling, squeezing, paste, dispensing, demo, mirror or VFX. The pack is visible on the counter in most scenes; the final scene settles on the pack on the counter with a spoken CTA. Keep each scene to ONE simple continuous action — the simpler and stiller the shot, the cleaner it renders.',
