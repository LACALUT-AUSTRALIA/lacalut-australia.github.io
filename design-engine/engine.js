...
"""`;
  ...
  const SB_LINT_RULES = [
    ...
    { re:/\b(zoom\w*|push\w*|close-?up|macro)\b[^.]{0,35}\b(plant|pot|towel|mug|cup|glass|tumbler|window|prop|shelf)\b/i, f:['visual','motion'], msg:'camera feature on a PROP — every beat must serve the product story; cut the prop beat and give the time to the problem or the pack' },
    ...
    { re:/\b(snap[- ]?zoom|zoom(s|ing)?|close-?up|macro|crash zoom)\b[^.]{0,45}\b(pack|tube|box|product)\b|\b(pack|tube|box|product)\b[^.]{0,45}\b(snap[- ]?zoom|close-?up|macro)\b/i, f:['visual','motion'], msg:'zoom/close-up on the pack — the pack stays UNDER A THIRD of the frame, straight-on and static (zooms warp the label lettering); hold the camera steady instead' }
  ];
  ...
  const SB_ONE_WORLD_RE = /(bathroom[^.]{0,80}kitchen|kitchen[^.]{0,80}bathroom|bedroom[^.]{0,80}(kitchen|bathroom)|(kitchen|bathroom)[^.]{0,80}bedroom|adjacent (kitchen|bathroom|bedroom|room|corner|space)|two (rooms|locations|settings|spaces))/i;
  function lintStoryboard(sb, styleId, typeId){
    const out=[];
    const lofi = String(styleId||'')==='lofi_native';
    const contrastType = typeId==='vs'||typeId==='transform';
