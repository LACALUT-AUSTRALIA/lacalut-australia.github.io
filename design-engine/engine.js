...
(function (global) {
  ...
  function buildMultiPrompt(opts){
    ...
    p += `CONSISTENT PRODUCT FORM (mandatory, non-negotiable): show every product in the SAME format — either ALL as tubes/bottles OR ALL as cartons/boxes across the whole line-up. NEVER mix some tubes and some boxes in the one image. IMPORTANT: even if a supplied reference photo shows a box AND a tube together, pick ONE form factor only and render just that form for every product — do NOT reproduce both. Keep the form uniform for all ${names.length} products so the range reads as one clean, cohesive set. `;
