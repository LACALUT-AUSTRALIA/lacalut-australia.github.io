...
"""`;
  ...
  const SB_LINT_RULES = [
    ...
    { re:/\b(effective(ly)?|firms|firming|firmness|strengthens?|supports?|protect(s|ion|ive|ed)?|prevents?|fights?|combats?|repairs?|treat(s|ments?)?|cures?|heals?|regenerat\w*|clinical(ly)?|proven|health(y|ier)?(?![- ]look))\b/i, f:['vo','text'], msg:'efficacy/therapeutic wording — cosmetic feel-language only ("feels firm", "cared-for")' },
    ...
    { re:/\b(?<!feels )(?<!feel )firm\w*[^.]{0,28}?gums?\b/i, f:['vo','text'], msg:'bare firming-result phrase ("firm gums") — feel-language only: "gums that FEEL firm"' },
