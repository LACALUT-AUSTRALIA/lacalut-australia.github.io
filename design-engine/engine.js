...
"""`;
  ...
  const SB_LINT_RULES = [
    ...
    { re:/\b(close-?up|CU|ECU|macro|tight shot)\b[^.]{0,80}\b(hands?|fingers?|knuckles?)\b|\bhands?\b[^.]{0,20}\bfills? the frame\b/i, f:['visual','motion'], msg:'hand close-up — hands are the worst AI subject; hands appear only inside mid-shots, never as the subject of a frame' },
    { re:/\b(holds?|holding|lifts?|lifting|raises?|raising|presents?|presenting|picks? up|carr(y|ies|ying))\b[^.]{0,30}\b(pack|tube|box|bottle|product)\b(?![^.]{0,25}\b(steady|in place|on the (counter|surface|slab|bench)|standing)\b)|\b(pack|tube|box|bottle)\b[^.]{0,20}\b(in|into) (her|his|one|a) hand\b/i, f:['visual','motion'], msg:'pack held/lifted in a hand — a hand-held pack drifts toward the lens and garbles the label (the 24/09 S1 fault); the pack always STANDS on a surface (a hand may press or steady it while it stays standing)' },
...
- NO COPY AUTOPILOT: never use the same distinctive word three or more times across the ad's hook, captions, VO and CTA — vary the wording (e.g. "cared-for" once or twice, never in every line). Short words count too: "firm", "care", "fresh" obey the same max-twice rule.
