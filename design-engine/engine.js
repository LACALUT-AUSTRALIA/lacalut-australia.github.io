...
(function (global) {
  ...
  function buildMultiPrompt(opts){
    ...
    p += `CONSISTENT PRODUCT FORM (mandatory): show every product in the SAME format — either ALL as tubes/bottles OR ALL as cartons/boxes across the whole line-up. NEVER mix some tubes and some boxes in the one image; pick a single form factor and keep it uniform for all ${names.length} products so the range reads as one clean, cohesive set. `;
...
- Keep icon'd badges (globe = countries, medal/ribbon = heritage/since-1925, shield = protection, sparkle = stain-lift) but VARY their shape — pills, rectangles, square tags, ribbons — never all identical round pills. Never flatten to plain unstyled text.
