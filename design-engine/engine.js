...
"""`;
  ...
  function lintStoryboard(sb, styleId, typeId){
    ...
    (sb.scenes||[]).forEach((sc,i)=>{
      ...
      const xref=(String(sc.visual||'')+' '+String(sc.motion||'')).match(/\b(previous|prior|last|earlier|next|opening|first) scene\b|\bscene \d\b|\bcontinu(ing|es?) from\b|\bas before\b|\bsame as (the )?(previous|last)\b/i);
