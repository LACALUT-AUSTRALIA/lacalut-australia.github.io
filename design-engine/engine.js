...
"""`;
  ...
  const SB_LINT_RULES = [
    ...
    { re:/(for|gives?|provides?|delivers?|leaves?)[^.]{0,30}firm\w*[^.]{0,15}gums/i, f:['vo','text'], msg:'implies a physical firming RESULT — cosmetic feel-language only: say "gums that FEEL firm and cared-for"' },
    { re:/(snap[- ]?zoom|zoom(s|ing)?|close-?up|macro|crash zoom)[^.]{0,45}(pack|tube|box|product)|(pack|tube|box|product)[^.]{0,45}(snap[- ]?zoom|close-?up|macro)/i, f:['visual','motion'], msg:'zoom/close-up on the pack — the pack stays UNDER A THIRD of the frame, straight-on and static (zooms warp the label lettering); hold the camera steady instead' }
  ];
