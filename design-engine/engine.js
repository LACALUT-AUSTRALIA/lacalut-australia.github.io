...
"""`;
  ...
  const SB_LINT_RULES = [
    ...
    { re:/\b(water|liquid|stream|droplets?|drips?|pours?|pouring)\b[^.]{0,55}\b(crystals?(?![- ]clear)|shards?|minerals?)\b|\b(crystals?|shards?)\b[^.]{0,40}\b(water|stream|drips?)\b/i, f:['visual','motion'], msg:'water interacting with crystals/minerals — the rejected rock-water fault; water exists ONLY as tap-into-sink or a drinking glass' },
    ...
    { re:/\b(visual (representation|metaphor)|represent(s|ing)? (care|confidence|freshness|comfort|trust)|abstract (concept|arrangement|swirl))\b/i, f:['visual','motion'], msg:'abstract concept written as a visual — scenes show real, concrete, nameable objects only' },
